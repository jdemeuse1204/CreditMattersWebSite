...
export const lookup = {
  ...
  getCreditBureauStatuses: function() {
    return new Promise((resolve, reject) => {
      ...
      model.read().then(() => {
        let data = toArrayFromKendoDataSource(model.data());
        resolve(data);
      }).fail(() => {
        reject();
      });
...
export const account = {
  ...
  enableTwoFactorAuthentication: function(timeOut) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'EnableTwoFactorAuthentication'), {
      timeOut: timeOut
    });
  },
  ...
  canAccessManagementPages: function() {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'CanAccessManagementPages'), null, {
      shouldRedirect: false
    });
  },
  sendAuthorizationCode: function(username) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'SendDeviceAuthorizationCode'), {
      username: username
    });
  },
  login: function(username, password) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'Login'), {
      username: username,
      password: password
    }, {
      addCustomClaimsCallback: addCustomClaimsCallback
    });
  },
  ...
  getSecurityQuestions: function(username) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'GetSecurityQuestions'), {
      Username: username
    });
  },
  ...
  isUidValid: function(uid) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'IsUidValid'), {
      Uid: uid
    });
  },
  verifyVerificationCode: function(username, password, code) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'VerifyVerificationCode'), {
      Username: username,
      Password: password,
      PIN: code
    }, {
  ...
  saveEmailAddresses: function(alternateEmailAddress) {
    return post(_getUrl(ACCOUNT_CONTROLLER, 'SaveEmailAddresses'), {
...
export const register = {
  ...
  authorizeUser: function(uid) {
    return post(_getUrl(REGISTER_CONTROLLER, 'AuthorizeUser'), {
      authorizationString: uid
    });
  },
  resendRegistrationEmail: function(emailAddress) {
    return post(_getUrl(REGISTER_CONTROLLER, 'ResendRegistrationEmail'), {
      Email: emailAddress
    });
  }
};
...
export const management = {
  ...
  approveDisputeReason: function(disputeReasonId, creditItemId) {
    return post(_getUrl(MANAGEMENT_CONTROLLER, 'ApproveDisputeReason'), {
      CreditBureauEntryId: creditItemId,
      DisputeReasonId: disputeReasonId
    });
  },
  getCustomerDipsuteStatementAddress: function(customerDisputeStatementId) {
    return get(_getUrl(MANAGEMENT_CONTROLLER, 'GetCustomerDipsuteStatementAddress'), {
      id: customerDisputeStatementId
    });
  },
  ...
  getCreditItems: function(creditBureau) {
    return get(_getUrl(MANAGEMENT_CONTROLLER, 'GetCreditItems'), {
      bureau: constants.getCreditBureauId(creditBureau)
    });
  },
  getCreditItemsThatNeedDisputeReasonAcceptance: function(creditBureau) {
    return get(_getUrl(MANAGEMENT_CONTROLLER, 'GetCreditItemsThatNeedDisputeReasonAcceptance'), {
      bureau: constants.getCreditBureauId(creditBureau)
    });
  },
  getCustomerDisputeStatements: function() {
    return get(_getUrl(MANAGEMENT_CONTROLLER, 'GetCustomerDisputeStatements'), {
      bureau: constants.getCreditBureauId(creditBureau)
    });
  },
...
function addCustomClaimsCallback(headersObject) {
  ...
  let token = getDeviceAuthorizationToken() || '';
